...
The application lives in [`goat-os/`](./goat-os). This repository also retains the original GOAT SDK history because goatOS is built on top of GOAT SDK.
...
The default database URL in `goat-os/.env.example` is:
...
```text
goat-os/src/app            Next.js pages and API routes
goat-os/src/components     Reusable UI modules
goat-os/src/lib/agents     Agent prompts, schemas, and runtime
goat-os/src/lib/goat       GOAT SDK wrapper and mock execution layer
goat-os/src/lib/policy     Policy engine and governance mandates
goat-os/src/lib/tokens     Token adapters, profiles, and risk scoring
goat-os/src/lib/theses     Thesis registry helpers
goat-os/src/lib/bounties   Bounty creation and payout flow
goat-os/src/lib/approvals  Approval request lifecycle
goat-os/src/lib/logs       Public action log
goat-os/tests              Critical unit tests
...
2. Add or update the system prompt in `goat-os/src/lib/agents/prompts.ts`.
3. Add a typed runtime function in `goat-os/src/lib/agents/runtime.ts`.
...
1. Create the adapter in `goat-os/src/lib/tokens/adapters`.
